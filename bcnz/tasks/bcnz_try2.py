...
#        b = np.einsum('gf,gf,zsf->gzs', var_inv, flux, f_mod)
        ...
        # For simplicify we first things to the Halpha amplitude.
        ind_Halpha = list(f_mod.sed).index('line_Halpha')
        isline = np.array([x.startswith('line_') for x in f_mod.sed.values])

#        Q = np.zeros(len(isline))
