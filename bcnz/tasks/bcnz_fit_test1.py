...
class bcnz_fit_test1:
    ...
    def chi2_min(self, f_mod, data_df):
        ...
        Smin = np.zeros(self.config['Niter'])
        Smax = np.zeros(self.config['Niter'])
        Smed = np.zeros(self.config['Niter'])
        S50 = np.zeros(self.config['Niter'])
        t1 = time.time()
        for i in range(self.config['Niter']):
            ...
            #S = pd.Series((m0-1).flatten())
            S = pd.Series((m0[:,:,0] - 1.).flatten())
            Smin[i] = S.min()
            Smax[i] = S.max()
            Smed[i] = S.abs().median()

            T = np.abs(np.log10(m0))

            S50[i] = S.abs().quantile(0.50)
        ...
        xvals = np.arange(self.config['Niter'])
