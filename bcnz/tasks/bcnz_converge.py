...
#    """Testing adding extinction."""
    """Test setting a better initial condition so the system converge
       faster.
    """
    ...
    def get_init(self, f_mod):
        
        import time
        t1 = time.time()

        fmod2 = f_mod.sum(dim='band')
        best = np.zeros((len(fmod2.z), len(fmod2.model))) #, len(f_mod.model)))

        for i,zs in enumerate(fmod2.z):
            y = fmod2.sel(z=zs).values

            B = np.outer(y, y)

            m,v = np.linalg.eigh(B)
            ind_vec = np.argmax(np.abs(m))

            best[i] = v[:, ind_vec]

        print('time best vec', time.time() - t1)
