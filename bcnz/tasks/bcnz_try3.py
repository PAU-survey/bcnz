...
#  'use_ext': 'If including extinction',
  'k_scale': 'Scaling of the fluxes'
}

class bcnz_try3:
    # Experiment with withing another amplitude...
    ...
    def _core_minimize(self, f_mod, data_df, zs): #, mabs_df):
...
#        F = np.einsum('zsf,gzs->gzf', f_mod, v)
        ...
        k_scale = self.config['k_scale']
        chi2 = var_inv*(flux*k_scale - F)**2
        ...
        return chi2, norm
        #return chi2, norm, flux, flux_err, f_mod
        return f_mod, v, flux, var_inv 

    def chi2_min(self, f_mod, data_df, zs): #, mabs_df):
        return self._core_minimize(f_mod, data_df, zs)
        
    def XXchi2_min(self, f_mod, data_df, zs): #, mabs_df):

        from matplotlib import pyplot as plt
        f_mod, v, flux, var_inv = self._core_minimize(f_mod, data_df, zs)

        F = np.einsum('zfs,gzs->gzf', f_mod, v)

        from matplotlib import pyplot as plt
        coords = {'gal': np.array(data_df.index), 'band': f_mod.band, 'z': f_mod.z}
        F = xr.DataArray(F, coords=coords, dims=('gal', 'z', 'band'))

        chi2 = var_inv*(flux - F)**2
        for i in range(len(data_df)):
            for k in [0.8, 0.9, 1.0, 1.1]:
                chi2 = var_inv*(k*flux - F)**2

                pb = np.exp(-0.5*chi2.sum(dim='band'))
                pb = pb / (1e-100 + pb.sum(dim='z'))

                lbl = 'k: {}'.format(k)
                plt.plot(pb.z, pb[i], label=lbl)
