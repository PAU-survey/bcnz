...
class bcnz_try11:
    ...
    # By now testing what happens if we correct the formula which is
    # used when minimizing the difference between the two systems.
...
#        S1 = (var_inv_NB*flux_NB).sum(dim='band') # for bcnz_try10...
...
#
# Old and wrong code for estimating the scaling between the two systems.
...
#                k = np.clip(k, 0.1, 10)
                ...
                vmodel = np.einsum('zfs,gzs->gzf', f_mod_NB, v)

                T1 = np.einsum('gf,gzf,gf->gz', var_inv_NB, vmodel, flux_NB)
                T2 = np.einsum('gf,gzf->gz', var_inv_NB, vmodel**2)
...
#        return chi2, norm
    ...
    def run(self):
        ...
        chunksize = 12 # For not matching with number of templates...
