...
#  'use_ext': 'If including extinction'
  'Nfourier_rm': 'Number of fourier components to remove'
}

class bcnz_fourier:
    """Fitting a linear combination to the observations."""

    # Experimenting with removing some Fourier components...
    ...
    def flux_clip(self, flux, flux_err):

        fft = np.fft.rfft(flux, axis=1)
        Nrm = self.config['Nfourier_rm']
        fft[:,:Nrm] = 0.

        flux_changed = np.fft.irfft(fft, axis=1)
        flux.values = flux_changed
...
#        ipdb.set_trace()
    ...
    def clip_model(self, f_mod):
        """Remove the Nfourier_rm first Fourier components."""
   
        # This code was simpler when using side effects. 
        fft = np.fft.rfft(f_mod, axis=1)
        Nrm = self.config['Nfourier_rm']
        fft[:,:Nrm,:] = 0.

        flux_changed = np.fft.irfft(fft, axis=1)
        f_mod.values = flux_changed
