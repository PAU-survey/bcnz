...
#        ipdb.set_trace()
    ...
    def decompose_model(self, f_mod):
        from sklearn.decomposition import NMF

        f_mod = np.clip(f_mod, 0., np.infty)

        new_fmod = np.zeros_like(f_mod)
        for i in range(len(f_mod.z)):
            inst = NMF()

#            new_fmod[i] = inst.fit_transform(f_mod[i].T).T
            new_fmod[i] = inst.fit_transform(f_mod[i])


        new_fmod = xr.DataArray(new_fmod, dims=f_mod.dims, coords=f_mod.coords)
    ...
    def run(self):
        ...
        f_mod = self.decompose_model(f_mod)
