...
descr = {
  ...
  'scale_version': 'How to scale the amplitude'
}

class inter_calib_xfree:
    ...
    def minimize_free(self, f_mod, flux, flux_err):
        ...
        S1_out = (var_inv_NB*flux_NB).sum(dim='band')
        ...
        for i in range(self.config['Niter']):
            ...
            if 0 < i and i % self.config['Nskip'] == 0:
                sv = self.config['scale_version']
                if sv == 10:
                    S2 = np.einsum('gf,gfs,gs->g', var_inv_NB, f_mod_NB, v)
                    k = (S1_out / S2).values
                elif sv == 11:
                    vmodel = np.einsum('gfs,gs->gf', f_mod_NB, v)

                    S1 = np.einsum('gf,gs,gf->g', var_inv_NB, vmodel, flux_NB)
                    S2 = np.einsum('gf,gf->g', var_inv_NB, vmodel**2)
