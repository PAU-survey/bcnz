...
class bcnz_fit_lines:
    # Test changing the model backend and start to add lines.....
...
#        b = np.einsum('gf,gf,zsf->gzs', var_inv, flux, f_mod)
    ...
    def fix_fmod_format(self, f_mod):
        print('fix the format...')
        print('FIRST try...')

        import copy
        A = f_mod.reset_index()
        B = pd.DataFrame(A.to_dict(), columns=['z', 'sed', 'EBV', 'band', 'f_mod'])
        C = B.set_index(['z', 'sed', 'EBV', 'band'])
        D = C.to_xarray().f_mod
...
#        ipdb.set_trace()
    ...
    def run(self):
        ...
        if False:
            f_mod = self.job.f_mod.result
            f_mod = f_mod.reorder_levels(['z', 'sed', 'EBV', 'band'])
            f_mod = f_mod.to_xarray().f_mod
            f_mod = f_mod.stack(model=['sed', 'EBV'])
