...
#                ipdb.set_trace()
        ...
        # Very crude try..
        vA = xr.DataArray(v, dims=('gal', 'z', 'model'), coords={'gal':flux.gal, 'z':f_mod.z, 'model': f_mod.model})
        R = vA.sel(sed='lines') / vA.sel(sed='O')

        fac = self.config['fac']
        pb_norm = np.exp(-fac*np.abs(R-1)).values[:,:,0]
