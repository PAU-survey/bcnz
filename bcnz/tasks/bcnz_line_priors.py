...
#  'use_ext': 'If including extinction'
  'line_pr': 'Fraction which an emission line is allowed to vary.'
}

class bcnz_line_priors:
    """Fitting a linear combination to the observations."""

    # Experimenting with adding a prior to the emission line ratios..
    ...
    def chi2_min(self, f_mod_cont, f_mod_lines, data_df, zs): #, mabs_df):
...
#        b = np.einsum('gf,gf,zsf->gzs', var_inv, flux, f_mod)
        b_cont = np.einsum('gf,gf,zfs->gzs', var_inv, flux, f_mod_cont)
        b_lines = np.einsum('gf,gf,zfs->gzs', var_inv, flux, f_mod_lines)
        print('time b',  time.time() - t1)

        Ap_cont = np.where(A_cont > 0, A_cont, 0)
        An_cont = np.where(A_cont < 0, -A_cont, 0)

        v_cont = 100*np.ones_like(b_cont)
        v_lines = 0.01*np.ones_like(b_lines)
#        v_lines = np.zeros_like(b_lines)

        gal_id = np.array(data_df.index)
        coords = {'gal': gal_id, 'band': f_mod_cont.band, 'z': f_mod_cont.z}
        coords_norm = {'gal': gal_id, 'z': f_mod_cont.z, 'model': f_mod_cont.model}

#        if (Ap < 0).any():
#            Ap = np.clip(Ap, 1e-50, np.infty)

        # Adding the prior part..
        Q = np.diag((1./self.config['line_pr']**2)*np.ones(A_lines.shape[-1]))
        A_lines = A_lines[:,:,] + Q

        F_lines = np.einsum('zfs,gzs->gzf', f_mod_lines, v_lines) #cont)

        t1 = time.time()
        for i in range(self.config['Niter']):
            a_cont = np.einsum('gzst,gzt->gzs', Ap_cont, v_cont)

            b_delta_cont = np.einsum('gf, gzf, zfs -> gzs', var_inv, F_lines, f_mod_cont)

            m0_cont = (b_cont - b_delta_cont) / a_cont
            vn_cont = m0_cont*v_cont
...
#            adiff = np.abs(vn-v)
            ...
            # Quick test if this can actually be exactly solved in a reasonable
            # time...

            F_cont = np.einsum('zfs,gzs->gzf', f_mod_cont, v_cont)

#            tsolve = time.time()
#            for j in range(A_lines.shape[0]):
#                for k in range(A_lines.shape[1]):
#                    ipdb.set_trace()

            # And then make an iteration for the emission line corrections...
            a_lines = np.einsum('gzst,gzt->gzs', A_lines, v_lines)

#            ipdb.set_trace()
            b_delta_lines = np.einsum('gf, gzf, zfs -> gzs', var_inv, F_cont, f_mod_lines)

            m0_lines = (b_lines - b_delta_lines) / a_lines
            vn_lines = m0_lines*v_lines
        ...
        F_cont = np.einsum('zfs,gzs->gzf', f_mod_cont, v_cont)
        F_lines = np.einsum('zfs,gzs->gzf', f_mod_lines, v_lines)
...
#        F = np.einsum('zsf,gzs->gzf', f_mod, v)
        ...
        norm = xr.DataArray(v_cont, coords=coords_norm, dims=('gal','z','model'))
...
#        f_mod = f_mod.stack(model=['sed', 'EBV'])
        ...
        seds_line = list(filter(lambda x: x.startswith('line_'), f_mod.sed.values))
        seds_cont = list(set(f_mod.sed.values) - set(seds_line))
    ...
    def run(self):
        ...
        f_mod_cont, f_mod_lines, f_mod_full = self.fix_fmod_format(self.job.f_mod.result)
...
#            ipdb.set_trace()
            chi2, norm = f_algo(f_mod_cont, f_mod_lines, galcat, zs)
